    ...
    <header className={cn("sticky top-0 z-50 w-full glass border-b border-border/60", className)}>
      <div className="mx-auto grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-4 py-3 sm:flex sm:h-16 sm:max-w-7xl sm:justify-between sm:px-6 sm:py-0">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <Link to="/login" className="hidden text-sm text-muted-foreground hover:text-foreground sm:inline">Entrar</Link>
          <Link to="/cadastro" className="hidden rounded-full bg-gradient-to-r from-primary to-accent px-4 py-2 text-sm font-medium text-primary-foreground ring-glow transition-transform hover:-translate-y-0.5 sm:inline-flex">
            Começar
          </Link>
        ...
        <div className="border-t border-border/60 px-4 py-3 md:hidden">
          <ul className="flex flex-col gap-1">
            ...
            <li className="mt-2 flex gap-2 border-t border-border/60 pt-3">
              <Link to="/login" className="flex-1 rounded-full bg-secondary/60 px-4 py-2 text-center text-sm text-foreground">Entrar</Link>
              <Link to="/cadastro" className="flex-1 rounded-full bg-gradient-to-r from-primary to-accent px-4 py-2 text-center text-sm font-medium text-primary-foreground ring-glow">Começar</Link>
            </li>
          </ul>
        </div>
      )}

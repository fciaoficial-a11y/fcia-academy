...
function AdminTable() {
  const rows = ADMIN_RESOURCES.questoes as Array<Record<string, string | number>>;
  const cols = rows.length ? Object.keys(rows[0]) : [];
  return (
    <AdminShell>
      <PageHeader crumbs={[{ label: "Admin", to: "/admin" }, { label: "Questões" }]} eyebrow="Admin" title="Questões" description="Banco de questões."
